...
#'     , timestamp = c(
#'         "2015-03-14T09:26:53B", "2015-03-14T09:26:54B"
...
parse_date_time <- function(input_df
    ...
    if (!data.table::is.data.table(input_df)) {
        msg <- paste(
            "parse_date_time expects to receive a data.table object."
    ...
    if (!identical(class(date_cols), "character")) {
        msg <- paste(
            "The date_cols argument in parse_date_time expects",
    ...
    if (!all(date_cols %in% names(input_df))) {
        ...
        msg <- paste(
            "The following columns, which you passed to date_cols,",
    ...
    for (dateCol in date_cols) {
        ...
        dateTimes <- paste0(
            stringr::str_extract(dateVec, "^\\d{4}-\\d{2}-\\d{2}"), " ",
        ...
        dateTimes <- purrr::map2(
            ...
            , function(dateTime, timeZone) {
                as.POSIXct(dateTime, tz = timeZone)
            }
        )
